...
function getInitData() {
  ...
  code += ',news: API.pages.get({gid:1, pid: ID, need_html: 1})';
...
function GetFriends(){
    VK.Api.call('friends.get', {fields: "first_name, last_name, photo_medium"}, function(r) {
        if(r.response) {
            var out="<table>";
            for (var i=0;i< r.response.length;i++){
                out +="<tr><td><img height='50px' width='50px' src='"+r.response[i].photo_medium+"' /></td>";
                out +="<td>"+r.response[i].last_name+"</td>";
                out +="<td>"+r.response[i].first_name+"</td></tr>";
            }
            out +="</table>";
