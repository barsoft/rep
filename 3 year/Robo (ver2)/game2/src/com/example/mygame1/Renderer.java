...
public class Renderer implements GLEventListener {
	...
	public void init(GLAutoDrawable arg0) {
		...
		MaterialCreatorTool.loadTypes(this);
		...
		hud = new Hud("hud", -getMapWidth() / 2, -getMapHeight() / 2,
				getMapWidth(), getMapHeight(), null, this);

		hud.parent = cam;

		for (int i = 0; i < 30; i++) {
			backGround.add(new Background("back", i * (getMapWidth() + 200), 0,
					getMapWidth() + 200, getMapHeight(), background, this));
		}

		for (int i = 0; i < 30; i++) {
			for (int j = 0; j < 100; j++) {

				grounds.add(new Ground(i * blockWidth, getMapHeight()
						+ blockHeight * j, Ground.getTypeByName("ground"), this));
			}
		}
		...
		for (int i = 0; i < grounds.size() / 4; i++) {
			...
			grounds.add(new Ground(g.x, g.y, Ground.getTypeByName("stone"),
					this));
			grounds.remove(g);
		}
