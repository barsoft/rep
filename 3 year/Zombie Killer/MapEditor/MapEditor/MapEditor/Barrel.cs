...
namespace MapEditor
{
        ...
        internal class List2PropertyConverter : StringConverter
        {
                ...
                ITypeDescriptorContext context)
            {
                return new StandardValuesCollection(
                    BarrelType.Types.Name);
            }
        }

        public BarrelType(Game1 game1, String _textureName, int _width, int _height, int _health, int _power, String _name)
        {
            BarrelType.Types.Name.Add(_name);
    ...
    public class Barrel : Base
    {
        [TypeConverter(typeof(BarrelType.List2PropertyConverter))]
        [Category("Barrel")]
        public string Type
        {
            get
            ...
            {
                typeName = value;
                foreach (BarrelType bt in game1.map.barrelTypes)
                {
                    if (bt.Name==typeName)
                    {
        ...
        public Barrel(Game1 game1,string typeName, int x, int y, string name)
            : base(game1)
        {
